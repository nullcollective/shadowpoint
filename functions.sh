...
init () {
    ...
    # Check to make sure directory is created, if not, initialize it
...
send_message() {
    ...
    if ! [ -n "${encrypt_recip}" ]; then
        ...
        if ! [ -n "${message}" ]; then
            ...
            # Encrypt Message
            enc_message=$(echo "${message}" | age -r ${encrypt_recip} --armor | base64 -w0)
...
rcv_message() {
    ...
    if ! [ -f ${personal_key} ]; then
        ...
        if ! [ -n "${paste_key}" ]; then
            ...
            decrypted_msg=$(echo "${enc_message}" | base64 -d | age -d -i ${personal_key})
