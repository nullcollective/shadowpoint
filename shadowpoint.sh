...
#---------------- USER CONFIG ----------------#
SPARK_DIR="${HOME}/.shadowpoint"
personal_key="${SPARK_DIR}/key.txt"
recipient_file="${SPARK_DIR}/recipients.txt"
...
VERSION="0.2"
HEADER="SPARK ENCRYPTION GUI v${VERSION}"
...
init () {
    ...
    if ! [ -d "$HOME/.spark" ]; then
        echo "Initializing Spark Directory ~/.spark"
        mkdir ${SPARK_DIR}
    fi
}
...
encrypt_file () {
    ...
    spark_encrypt_recip=$(zenity --width=900 --height=500 --list --title="Select Recipient(s)" \
    ...
    if ! [ -n "${spark_encrypt_recip}" ]; then
        :
    else
        # File Selection
        spark_encrypt_filename=$(zenity --file-selection --title="Select a file to encrypt" 2>/dev/null)

        # Check if a file was selected
        if ! [ -n "${spark_encrypt_filename}" ]; then
            :
        else
            # ENCRYPTION PROCESS
            age -o "${spark_encrypt_filename}.age" -r ${spark_encrypt_recip} "${spark_encrypt_filename}"
            zenity --info --text="FILE ENCRYPTED: ${spark_encrypt_filename}.age"
        fi
    fi
}
...
decrypt_file () {
    # File Selection
    spark_decrypt_filename=$(zenity --file-selection --title="Select a file to encrypt" 2>/dev/null)
    file_type=$(file -b ${spark_decrypt_filename} | awk -F\, '{ print $1 }')

    # Check if a file was selected & is an "age encrypted file"
    if ! [ -n "${spark_decrypt_filename}" ]; then
        ...
        output_file=$(echo "${spark_decrypt_filename}" | sed 's/.age//g')
        directory="$(dirname ${output_file})"
        age -d -o "${output_file}" -i ${personal_key} "${spark_decrypt_filename}"

        # Display File Browser
        #$(${filebrowser} -w ${directory} & > /dev/null 2>&1)
...
send_message() {
    ...
    if ! [ -n "${spark_encrypt_recip}" ]; then
        ...
        if ! [ -n "${message}" ]; then
            ...
            # Encrypt Message
            enc_message=$(echo "${message}" | age -r ${spark_encrypt_recip} --armor)
...
reset () {
    zenity --warning \
    --text="Please manually delete the directory \"${SPARK_DIR}\""
}

cmd_prompt () {
    spark_cmd=$(zenity --width=900 --height=500 --list --title="${HEADER}" \
